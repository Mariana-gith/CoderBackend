...
class ContenedorMongodb{
    ...
    async upDateById(nuevoElemnto){
        try{
            cambiarKey(nuevoElemnto,'id','_id')
            let {p,pModificado} = await this.collection.replaceOne({'_id':nuevoElemnto._id},nuevoElemnto)
            if(p === 0 || pModificado ===0){
                throw new Error(`No se puede actualizar`)
            }else{
                cambiarKey(nuevoElemnto,'_id','id')
                borrarKey(nuevoElemnto,'__v')
                return objeto(nuevoElemnto)
            }
        }catch(error){
